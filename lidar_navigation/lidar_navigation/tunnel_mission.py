...
class TunnelMission(Node):
    ...
    def get_relative_coords(self, pose: PoseStamped, rel_x, rel_y, rel_angle = 0.0):
        ...
        rel_pose.pose.position.x = pose.pose.position.x + rel_x * cos(angle / 57.3) + rel_y * sin((angle + 90) / 57.3)
        rel_pose.pose.position.y = pose.pose.position.y + rel_x * sin(angle / 57.3) + rel_y * cos((angle + 90) / 57.3)
